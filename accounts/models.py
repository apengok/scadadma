...
class MyUserManager(BaseUserManager):
    def create_user(self, user_name, password=None):
        """
        Creates and saves a MyUser with the given email and password.
        """
        if not user_name:
            raise ValueError('MyUsers must have an user_name')
...
class MyUser(AbstractBaseUser,PermissionsMixin):
    ...
    def user_list_queryset(self):
        ...
        user_self = MyUser.objects.filter(user_name=self.user_name)
        userlist = user_self
        #下级组织的用户
